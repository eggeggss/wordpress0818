...
function twentyten_continue_reading_link() {
	return ' <br /><a href="'. get_permalink() . '">' . __( '繼續閱讀 <span class="meta-nav">»</span>', 'twentyten' ) . '</a>';
}
endif;
...
function twentyten_posted_on() {
	printf( __( '<span class="%1$s">發佈於：</span> %2$s <span class="meta-sep"> | 作者：</span> %3$s', 'twentyten' ),
...
function twentyten_posted_on_lp() {
	printf( __( '<span class="%1$s">發佈於：</span> %2$s ', 'twentyten' ),
		'meta-prep meta-prep-author',
		sprintf( '<a href="%1$s" title="%2$s" rel="bookmark"><span class="entry-date">%3$s</span></a>',
			get_permalink(),
			esc_attr( get_the_time() ),
...
function twentyten_posted_in() {
	...
	if ( $tag_list ) {
		$posted_in = __( '分類：%1$s 標籤：%2$s', 'twentyten' );
	} elseif ( is_object_in_taxonomy( get_post_type(), 'category' ) ) {
		$posted_in = __( '分類：%1$s', 'twentyten' );
	} else {
